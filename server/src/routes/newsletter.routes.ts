...
import { subscribeNewsletter, pastNewsletters, unsubscribeNewsletter } from '../controllers/newsletter.controller';

const router = Router();

router.post('/subscribe', writeLimiter, validate(subscribeNewsletterSchema), subscribeNewsletter);
router.post('/unsubscribe', writeLimiter, validate(subscribeNewsletterSchema), unsubscribeNewsletter);
